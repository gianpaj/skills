...
- Get the closing rate for that date:
...
Find first blank row after last data entry. **Data must be contiguous - no
gaps.**
...
**Important:** Invoice shows total WITH VAT. Calculate amounts without VAT
first.
...
Format:
`|Date|InvoiceNum|Description|Vendor|Category|Amount|Tax%|VATAmount|TotalWithVAT`
...
- **Solution:** Use decimal format (2.48), sheet's number formatting adds €
  automatically
...
See `SKILL.md` for comprehensive workflow and `references/vendors.md` for vendor
mappings.
